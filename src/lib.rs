...
pub mod event;
/// The async layer: [`exec::block_on`], the task `Waker`, `Sleep`. A `.await` that returns `Pending`
/// gives up the slice exactly like a blocking call does.
pub mod exec;
/// `embedded-hal` / `embedded-hal-async` adapters: delays, the blocking facade over an async driver,
/// and a kernel-mutex device for a bus shared between tasks.
pub mod hal;
/// `embedded-io` / `embedded-io-async` byte streams on top of an ISR-driven [`event::Signal`].
pub mod io;
pub mod ring;
pub mod scheduler;
pub mod smp;
